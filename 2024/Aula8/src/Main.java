	...
	public static void main(String args[]) {
		...
		do {
			...
			if(option == 1) {
				...
				for(Conta itemConta: contas) {
					if(itemConta.numero == nContaSaque) {
						System.out.println("Saldo atual R$" +itemConta.saldo);
						System.out.println("Digite o valor do saque:");
						double valorSaque = sc.nextDouble();
						itemConta.sacar(valorSaque);
						System.out.println("Saldo atualizado R$" +itemConta.saldo);
					}
				}
			}else if(option== 4) {
				...
				for(Conta itemConta: contas) {
					if(itemConta.numero == nContaDeposito) {
						System.out.println("Saldo atual R$" +itemConta.saldo);
				...
				Conta cOrigem, cDestino = new Conta();// duas contas
				for(Conta itemConta: contas) {
					if(itemConta.numero == nContaOrigem) {
						cOrigem = itemConta;
					}else if(itemConta.numero == nContaDestino) {
						cDestino = itemConta;
					}
				}
				//Ler valor
